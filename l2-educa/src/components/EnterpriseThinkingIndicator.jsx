...
import './ThinkingIndicatorFallback.css';

/**
 * ENTERPRISE-LEVEL Thinking Indicator
 * Robust, always-visible, multi-fallback system
 * 
 * Features:
 * - Multiple render modes (primary + 3 fallbacks)
 * - Self-diagnostic system
 * - Performance monitoring
 * - Accessibility compliant
 * - Error recovery
 */
const EnterpriseThinkingIndicator = ({ 
  mode = 'primary',
  forceVisible = false,
  onRenderError = null,
  debug = false 
  ...
  useEffect(() => {
    ...
    // Debug logging
    if (debug) {
      console.log('🧠 EnterpriseThinkingIndicator rendered:', {
        mode,
    ...
    // Error recovery: if not visible after 100ms, report
  ...
  // Primary mode: Full featured brain animation
  if (mode === 'primary') {
    return (
      <div 
        className="thinking-indicator enterprise-mode-primary" 
        data-testid="thinking-indicator"
        role="status"
        aria-live="polite"
        aria-label="Inteligência artificial pensando"
      >
  ...
  // Fallback 1: Simplified version (no SVG animations)
  if (mode === 'simplified') {
    return (
      <div className="thinking-indicator enterprise-mode-simplified">
        <div className="thinking-simple-spinner"></div>
        <span className="thinking-simple-text">Pensando...</span>
      </div>
    );
  }

  // Fallback 2: Minimal CSS-only version
  if (mode === 'minimal') {
    return (
      <div className="thinking-indicator enterprise-mode-minimal">
        <div className="thinking-minimal-dots">
          <span></span>
          <span></span>
          <span></span>
        </div>
  ...
  // Fallback 3: Text-only (ultimate fallback)
  return (
    <div className="thinking-indicator enterprise-mode-text">
      <span>🤔 Pensando...</span>
    </div>
  );
};

export default EnterpriseThinkingIndicator;
